...
describe('PublishMachine', () => {
  test('starts empty and invites a press', () => {
    ...
    expect(screen.getByText(`${CATALOGUE.length} left`)).toBeInTheDocument();
  });
  ...
  test('each press deals one real view count and updates the totals', () => {
    ...
    expect(screen.getByText(`${CATALOGUE.length - 1} left`)).toBeInTheDocument();
    ...
    fireEvent.click(publish);
    fireEvent.click(publish);
    expect(screen.getByText(`${CATALOGUE.length - 3} left`)).toBeInTheDocument();
  ...
  test('draws without replacement and ends on the real catalogue total', () => {
    render(<PublishMachine />);
    const publish = screen.getByRole('button', { name: /publish one/i });
    for (let i = 0; i < CATALOGUE.length; i += 1) fireEvent.click(publish);

    expect(screen.getByText('0 left')).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /that is the whole catalogue/i })
    ).toBeDisabled();
    ...
    expect(within(totals).getByText(total.toLocaleString())).toBeInTheDocument();
    expect(screen.getByText(/that is all 48/i)).toBeInTheDocument();
  });
